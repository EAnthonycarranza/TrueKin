/**
 * DesignerPanel – Main admin T-shirt designer with full editing tools.
 *
 * Architecture:
...
export default function DesignerPanel({
  ...
  useEffect(() => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (data.backObjects && backCanvasRef.current) {
          backCanvasRef.current.loadObjects(data.backObjects);
        }
        if (data.frontTexture) setFrontTexture(data.frontTexture);
        if (data.backTexture) setBackTexture(data.backTexture);
      }, 300);
    } catch {
      // Invalid design data
    }
  ...
  // --- Sync Fabric canvas -> 3D texture when designs change ---
  const handleDesignChange = useCallback((view) => {
    const ref = view === 'front' ? frontCanvasRef : backCanvasRef;
    const setter = view === 'front' ? setFrontTexture : setBackTexture;
    if (ref.current) {
      const dataURL = ref.current.getTextureDataURL();
  ...
  const handleSnapshot = async () => {
    ...
    try {
      ...
      if (onSnapshot) onSnapshot(blob, getDesignState());
    } catch (err) {
  ...
  // --- Snapshot (capture 2D canvas with t-shirt background as product image) ---
  const handleCapture2D = () => {
    setCapturing2D(true);
    try {
      const dataURL = activeCanvasRef.current?.getSnapshotDataURL();
      if (!dataURL) throw new Error('No canvas content');
      const blob = dataURLToBlob(dataURL);
      if (onSnapshot) onSnapshot(blob, getDesignState());
    } catch (err) {
      console.error('2D capture failed:', err);
    } finally {
      setCapturing2D(false);
    }
  };
  ...
  const handleSave = async () => {
    try {
      const canvas = threeCanvasRef.current?.getCanvas();
      const blob = await captureCanvasBlob(canvas);
      if (onSave) onSave(blob, getDesignState());
    } catch (err) {
    ...
    <div style={s.toolContent}>
      ...
      <div style={s.curvedBox}>
        ...
        <div style={s.editRow}>
          ...
          <span style={{ fontSize: 11, minWidth: 32, textAlign: 'right' }}>{curveAmount}&deg;</span>
        </div>
        <div style={s.editRow}>
      ...
      <div style={s.propsPanel}>
        ...
        <div style={s.effectRow}>
          ...
          <input type="range" min="-180" max="180" value={editCurveAmount}
            ...
            style={{ flex: 1 }}
          />
          <span style={s.effectValue}>{editCurveAmount}&deg;</span>
        </div>
      </div>
    );
  };
  ...
  return (
    <div style={s.wrapper}>
      ...
      <div style={s.leftColumn}>
        ...
        <div style={s.shirtStyleToggle}>
          <div style={s.unisexSeal}>
        ...
        <div style={s.colorPickerGroupLeft}>
        ...
        {/* Action buttons under 3D preview */}
        <div style={s.actions}>
          <button
            type="button"
          ...
          >
            <Camera size={16} />
            {capturing ? 'Capturing...' : '3D Snapshot'}
          </button>
          <button
            type="button"
            className="btn btn-secondary"
            onClick={handleCapture2D}
            disabled={capturing2D}
            style={s.actionBtn}
          >
            <ImagePlus size={16} />
            {capturing2D ? 'Capturing...' : '2D Snapshot'}
          </button>
          <button
            type="button"
      ...
      <div style={s.rightColumn}>
        ...
        {/* 2D Canvas (Fabric.js) with floating delete bar */}
        ...
        <p style={s.hint}>
          Click tool tabs above to add text, shapes, clip art, and images. The 3D preview updates in real-time.
        </p>
      </div>
    </div>
  );
...
const s = {
  ...
  // Color picker
  colorSwatches: { display: 'flex', flexWrap: 'wrap', gap: 3 },
  colorSwatch: {
    width: 18,
    height: 18,
    borderRadius: '50%',
