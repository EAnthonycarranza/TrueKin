...
function initialBuilder(quote) {
  ...
  return {
    ...
    productPreview: saved.productPreview || null,
    ...
    >
      ...
      <div className="qb-layout">
        <div className="qb-controls">
          ...
          <section className="card qb-section"><h2>Product preview</h2><p className="qb-help">Show the exact product or blank being proposed. Choose a catalog image or upload a custom preview; this appears in the document and customer email.</p><label className="qb-preview-select">Preview source<select value={builder.productPreview?.sourceProductId || (builder.productPreview ? 'custom' : '')} onChange={(event) => selectProductPreview(event.target.value)}><option value="">No product preview</option><option value="custom">Custom upload</option>{catalogProducts.map((product) => <option key={product._id} value={product._id}>{product.title}{product.active ? '' : ' · inactive'}</option>)}</select></label>{builder.productPreview && <><div className="qb-product-preview">{previewImage ? <img src={previewImage} alt={builder.productPreview.title || 'Product preview'} /> : <div className="qb-product-placeholder">Upload a product image</div>}<div><label>Product title<input maxLength="160" value={builder.productPreview.title || ''} onChange={(event) => update('productPreview', { ...builder.productPreview, title: event.target.value })} /></label><label>Description<textarea rows="3" maxLength="700" value={builder.productPreview.description || ''} onChange={(event) => update('productPreview', { ...builder.productPreview, description: event.target.value })} /></label>{!builder.productPreview.sourceProductId && <label className="qb-upload qb-product-upload"><FileImage size={14} /> {previewImage ? 'Replace image' : 'Upload product image'}<input type="file" accept="image/png,image/jpeg,image/webp" hidden onChange={(event) => { selectProductImage(event.target.files?.[0]); event.target.value = ''; }} /></label>}</div></div></>}</section>
